      ...
      {showBackButton && (
        <div className='pr-4 h-5 border-r-[0.5px] mr-4 lg:-ml-[34px]'>
          <Link href="/" className="group/back inline-block  text-secondary transition-colors hover:text-primary focus-visible:text-primary" aria-label="Go to home">
            <ArrowLeft width={16} height={16} className='transition-transform group-hover/back:-translate-x-1.5' />
          </Link>
        </div>
      )}

      <span className=" leading-none transition-[opacity,transform] ease-out duration-300 font-serif opacity-0 text-ellipsis whitespace-nowrap overflow-clip translate-y-2 group-data-[visible=true]:opacity-100 group-data-[visible=true]:translate-y-0">{title}</span>
    </div>
  </header>
)
