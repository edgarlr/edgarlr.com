...
export const Header = ({ title, showBackButton = false }: Props) => (
  ...
  >
    <div className={cn("w-full h-5 max-lg:px-6 flex gap-[22px] md:gap-9 flex-nowrap items-center max-w-[70ch] mx-auto", {
      ...
      <span className=" leading-none pt-1 transition-[opacity,transform] ease-out duration-300 font-serif italic opacity-0 text-ellipsis whitespace-nowrap overflow-clip translate-y-2 group-data-[visible=true]:opacity-100 group-data-[visible=true]:translate-y-0">
        {title}
      </span>
    </div>
  </header>
